...
class GAjax extends GObject {
    ...
    isValid(_data) {
        if(_data != "") {
            ...
            if(!lDom.loadXml(_data)) {
                this.m_dataLogs.addError("La r̩ponse n'est pas du XML valide.");
                return false;
            }
            else if(lDom.m_node.nodeName != "rdv") {
                this.m_dataLogs.addError("Le noeud racine de la r̩ponse XML n'est pas valide.");
                return false;
            }
        }
        return true;
    ...
    call(_module, _method, _params, _callback, _type) {
        ...
        if(_method == "") {
            this.m_logs.addError("La m̩thode est obligatoire.");            
            return false;
        }
        var lDom = new GCode();
        lDom.createDoc();
    ...
    callServer(_data, _callback) {
        if(_data == "") {
            this.m_logs.addError("La donn̩e est obligatoire.");            
            return false;
        }
